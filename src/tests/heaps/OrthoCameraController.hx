...
class OrthoCameraController extends CameraController {
	...
	public function setOrthoBounds(size:Float, near:Float, far:Float, animate:Bool = false) {
		var scene = if (scene == null) getScene() else scene;
		if (scene == null) {
			throw "Not in scene";
		}

		setOrthoSize(size, animate);

		var orthoBounds = new Bounds();
		orthoBounds.xMin = -size * scene.camera.screenRatio;
		orthoBounds.xMax = size * scene.camera.screenRatio;
		orthoBounds.yMin = -size;
		orthoBounds.yMax = size;
		orthoBounds.zMin = near;
		orthoBounds.zMax = far;
		scene.camera.orthoBounds = orthoBounds;
	}
	...
	override public function loadFromCamera(animate = false) {
		...
		// if the ortho camera is not set, set it
		if (scene.camera.orthoBounds == null) {
			setOrthoBounds(targetOrthoSize, scene.camera.zNear, scene.camera.zFar, animate);
		}
		...
		setOrthoSize(orthoSize, animate);
