...
class ScriptLoader {
	...
	private static function _load(scriptName:String, onLoaded:Script->Void):Void {
		...
		js.Lib.dynamicImport(scriptPath).then(m -> {
			var instance:Script = createInstanceFromModule(m, scriptName);
			onLoaded(instance);
		}).catchError(err -> {
	...
	public static function load(scriptName:String, onLoaded:String->Script->Void):Void {
		...
		_load(scriptName, (script) -> {
			if (script != null) {
				...
				if (!TypeUtils.isDerivedFrom(Type.getClass(script), core.scripting.Script)) {
					Log.error("Script " + scriptName + " does not derive from Script");
					if (onLoaded != null) {
						onLoaded(scriptName, null); // Call the callback with the loaded instance (no hot reload, so no previous instance)
					}
					return;
				}
